...
use slumber_core::{
    ...
    render::{OverrideKey, Overrides, Procedure},
};
use std::{
    fs,
    path::{Path, PathBuf},
        ...
        match body {
            ...
            | RecipeBody::FormMultipart { data } => {
                let inner = RecipeFieldTable::new(
                    ...
                    data.iter().map(|(field, value)| {
                        (
                            field.clone(),
                            value.clone(),
                            OverrideKey::Form(field.to_owned()),
                            FormRowToggleKey {
        ...
        match self {
            Self::Raw(component) => {
                ...
                if data.body.is_overridden() {
                    indexmap! {
                        OverrideKey::Body => data.body.value().into()
                    }
                } else {
                    indexmap! {}
                }
...
mod tests {
    ...
    fn test_persisted_override(
        ...
        let recipe = Recipe {
            body: Some(RecipeBody::Raw { data: "".into() }),
            ..Recipe::factory(())
        };
