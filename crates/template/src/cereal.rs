        ...
        D: Deserializer<'de>,
    {
        ...
        impl Visitor<'_> for TemplateVisitor {
            ...
            where
                E: Error,
            {
                // Minor hack: this error is always displayed followed the serde
                // key path. Our error will contain a pointer to the error
                // within the source text. For the pointer to line up, we have
                // to start on a fresh line.
                v.parse().map_err(|error| E::custom(format!("\n{error}")))
            }
        }
