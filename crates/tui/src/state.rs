...
mod tests {
    ...
    #[rstest]
    fn test_reload_collection(
        collection_file: CollectionFile,
        harness: TestHarness,
    ) {
        // Start with an empty collection
        fs::write(collection_file.path(), "export const requests = {};")
            .unwrap();
    ...
    /// Get a path to a collection file in a temporary directory. The file will
    /// be created but empty
    #[fixture]
    fn collection_file(temp_dir: TempDir) -> CollectionFile {
        let path = temp_dir.join("slumber.js");
