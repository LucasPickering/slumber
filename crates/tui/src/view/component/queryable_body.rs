    ...
    fn update_query(&mut self) {
        ...
        if command.is_empty() {
            ...
            tokio::spawn(async move {
                ...
                messages_tx
                    .send(Message::Local(Box::new(QueryComplete(result))));
            });
        }
    }
}
...
impl EventHandler for QueryableBody {
    fn update(&mut self, _: &mut UpdateContext, event: Event) -> Update {
        ...
        } else if let Some(QueryComplete(result)) =
            event.local::<QueryComplete>()
        {
            match result {
                Ok(stdout) => {
...
/// Callback event from executing the query command. Value is command stdout
#[derive(Debug)]
struct QueryComplete(anyhow::Result<Vec<u8>>);
...
mod tests {
    ...
    async fn test_text_body(
        ...
        // Wait for the command to finish, pass results back to the component
