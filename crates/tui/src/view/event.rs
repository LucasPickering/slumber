...
///
/// An [Event] is a subtype of [Message](crate::Message). It's pushed through
/// the same queue as all other messages, but events are handled by individual
/// components instead of at the root message handler.
#[derive(Debug)]
pub enum Event {
    /// See [BroadcastEvent]
