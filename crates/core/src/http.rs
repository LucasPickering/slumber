...
impl HttpEngine {
    ...
    pub async fn build(
        ...
        let (client, request) = seed.convert_error(future, context).await?;
    ...
    pub async fn build_url(
        ...
        let request = seed.convert_error(future, context).await?;
    ...
    pub async fn build_body(
        ...
        seed.convert_error(future, context).await
    }
    ...
    async fn render_query(
        ...
        let iter = self.query.iter().enumerate().filter_map(|(i, (k, v))| {
            ...
            Some(async move {
                Ok::<_, anyhow::Error>((
                    k.clone(),
                    template.render_string(context).await.context(format!(
