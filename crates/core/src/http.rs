...
use slumber_template::{
    Render, RenderError, RenderedOutput, StreamSource, Template,
};
...
impl Recipe {
    ...
    async fn render_query(
        ...
        let iter = merged.into_iter().map(async |((param, _), template)| {
            let value = template.render(context).await.map_err(|error| {
    ...
    async fn render_header(
        ...
        let value: Bytes =
            value_template.render(context).await.map_err(|error| {
                RequestBuildErrorKind::HeaderRender {
                ...
                let (username, password) = try_join!(
                    ...
                    async {
                        OptionFuture::from(
                            password
                                .as_ref()
                                .map(|password| password.render(context)),
                        )
                        .await
                        .transpose()
        ...
        match (&self.body, &options.body) {
            ...
            | (
                ...
                let output: RenderedOutput =
                    template.render(&context.stream()).await.expect("TODO");
            ...
            (Some(RecipeBody::FormUrlencoded(fields)), None) => {
                ...
                let iter = merged.into_iter().map(async |(field, template)| {
                    let value =
                        template.render(context).await.map_err(|error| {
