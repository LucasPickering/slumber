...
impl<K> QueryableBody<K> {
    ...
    fn export(&mut self) {
        ...
        self.spawn_command(command, body, |command, result| match result {
            ...
            Ok(_) => ViewContext::push_message(Message::Notify(format!(
                "`{command}` succeeded"
            ))),
            Err(error) => ViewContext::push_message(Message::Error { error }),
        });
    }
