    ...
    fn process_body(&mut self, request_body: ReferenceOr<RequestBody>) {
        ...
        let body = request_body
            ...
            .filter_map(|(mime, value)| {
                self.get_body(&mime, value).context("TODO").traced().ok()
            })
            // TODO sort
            .next();
    ...
    fn get_examples(
        ...
        let schema_example = media_type.schema.as_ref().and_then(|schema| {
            let schema = self
                .reference_resolver
                .resolve::<Schema>(
                    // TODO remove clone (modify resolve to take ref)
                    schema.clone(),
                )
                .with_context(|| {
                    format!(
    ...
    fn get_body(
        ...
        fn unwrap_object(
            ...
            if let serde_json::Value::Object(object) = value {
                Ok(object
                    .into_iter()
                    .map(|(key, value)| (key, Template::raw(value.to_string())))
                    .collect())
            } else {
...
pub mod tests {
    ...
    #[case::raw(
        ...
        RecipeBody::Raw(r#"{"field":"value"}"#.into()),
    )]
