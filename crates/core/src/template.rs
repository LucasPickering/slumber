...
use petit_js::{function::Function, Process, Value};
#[cfg(test)]
...
impl Renderer {
    ...
    /// template context attached. This should only be used from inside native
    /// functions, where the process has alreayd been initialize for template
    /// rendering but you don't have access to the wrapping `Renderer`.
    ...
    pub async fn render_value(
        ...
        match template {
            Template::Value(s) => Ok(s.clone().into()),
    ...
    pub async fn render_string(
        ...
        let s = self.render_value(template).await?.try_into_string()?;
        Ok(s.into())
    }
