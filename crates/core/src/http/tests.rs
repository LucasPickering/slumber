...
use rstest::rstest;
use serde_json::json;
use slumber_util::{Factory, assert_err, paths::get_repo_root, test_data_dir};
use std::ptr;
...
#[case::form_multipart_file(
    RecipeBody::FormMultipart(indexmap! {
        "file".into() => "{{ file(concat([test_data_dir, '/data.json'])) }}".into(),
    }),
...
async fn test_build_curl_body(
    ...
    let expected_arguments = expected_arguments
        .replace("{ROOT}", &get_repo_root().to_string_lossy());
